...
const deleteTask = async (filter) =>{
  try {
    let task = await dbService.findMany(Task,filter);
    if (task && task.length){
      ...
      let deleted  = await dbService.deleteMany(Task,filter);
      let response = { task :taskCnt + deleted, };
      return response; 
    } else {
      return {  task : 0 };
    }
...
const softDeleteTask = async (filter,updateBody) =>{  
  try {
    let task = await dbService.findMany(Task,filter, { id:1 });
    if (task.length){
      task = task.map((obj) => obj.id);

      const taskFilter = { '$or': [{ parentId : { '$in' : task } }] };
      const taskCnt = await dbService.updateMany(Task,taskFilter,updateBody);
      let updated = await dbService.updateMany(Task,filter,updateBody);

      let response = { task :taskCnt + updated, };
      return response;
    } else {
      return {  task : 0 };
    }
